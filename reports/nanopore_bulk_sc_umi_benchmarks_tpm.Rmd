---
title: "Nanopore bulk RNA-Seq and scRNA-Seq Sicelore UMI vs wf-single-cell UMI benchmarks (all >= 1 TPM transcripts)"
output:
  html_document:
    highlight: pygments
    df_print: paged
...
Select top transcripts for further analysis:
```{r, message = FALSE, warning = FALSE}
top_transcripts <- apply(wf_tpm_pseudobulk, 1, mean)
top_transcripts <- names(top_transcripts[top_transcripts >= min_tpm])
length(top_transcripts)
```
