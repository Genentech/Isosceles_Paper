---
title: "Nanopore IGROV-1 bulk RNA-Seq and scRNA-Seq mean relative difference benchmarks"
output:
  html_document:
    highlight: pygments
    df_print: paged
...
min_tpm <- 1
n_values <- c(100, 200, 500, 1000, 2000, 5000, 10000, 15000)
```
...
calculate_mean_rel_diff <- function(bulk_tpm, pseudobulk_tpm) {
    mean_rel_diff_values <- sapply(k_values, function(k_value) {
        mean_rel_diff(
            bulk_tpm,
            pseudobulk_tpm[names(bulk_tpm), as.character(k_value)]
        )
    })
    names(mean_rel_diff_values) = k_values
    return(mean_rel_diff_values)
}
top_n_mean_rel_diff <- function(bulk_tpm, pseudobulk_tpm, k_value) {
    bulk_tpm_values <- sort(bulk_tpm, decreasing = TRUE)
    pseudobulk_tpm_values <- pseudobulk_tpm[
        names(bulk_tpm_values), as.character(k_value)
    ]
    mean_rel_diff_values <- sapply(n_values, function(n) {
        if (length(bulk_tpm_values) < n) {
            return(NA)
        } else {
            return(mean_rel_diff(
                bulk_tpm_values[1:n], pseudobulk_tpm_values[1:n]
            ))
        }
    })
    return(mean_rel_diff_values)
...
Calculate Isosceles relative difference values:
```{r, message = FALSE, warning = FALSE}
isosceles_mean_rel_diff <- data.frame(
    tool = "Isosceles",
    platform = rep(c("Promethion", "MinION"), each = length(k_values)),
    k_value = rep(k_values, 2),
    mean_rel_diff = c(
        calculate_mean_rel_diff(
            isosceles_bulk_tpm_list[["Promethion"]],
            isosceles_pseudobulk_tpm
        ),
        calculate_mean_rel_diff(
            isosceles_bulk_tpm_list[["MinION"]],
            isosceles_pseudobulk_tpm
        )
    )
)
...
Calculate IsoQuant relative difference values:
```{r, message = FALSE, warning = FALSE}
isoquant_mean_rel_diff <- data.frame(
    tool = "IsoQuant",
    platform = rep(c("Promethion", "MinION"), each = length(k_values)),
    k_value = rep(k_values, 2),
    mean_rel_diff = c(
        calculate_mean_rel_diff(
            isoquant_bulk_tpm_list[["Promethion"]],
            isoquant_pseudobulk_tpm
        ),
        calculate_mean_rel_diff(
            isoquant_bulk_tpm_list[["MinION"]],
            isoquant_pseudobulk_tpm
        )
    )
)
...
Calculate FLAMES relative difference values:
```{r, message = FALSE, warning = FALSE}
flames_mean_rel_diff <- data.frame(
    tool = "FLAMES",
    platform = rep(c("Promethion", "MinION"), each = length(k_values)),
    k_value = rep(k_values, 2),
    mean_rel_diff = c(
        calculate_mean_rel_diff(
            flames_bulk_tpm_list[["Promethion"]],
            flames_pseudobulk_tpm
        ),
        calculate_mean_rel_diff(
            flames_bulk_tpm_list[["MinION"]],
            flames_pseudobulk_tpm
        )
    )
)
...
Calculate Sicelore relative difference values:
```{r, message = FALSE, warning = FALSE}
sicelore_mean_rel_diff <- data.frame(
    tool = "Sicelore",
    platform = rep(c("Promethion", "MinION"), each = length(k_values)),
    k_value = rep(k_values, 2),
    mean_rel_diff = c(
        calculate_mean_rel_diff(
            sicelore_bulk_tpm_list[["Promethion"]],
            sicelore_pseudobulk_tpm
        ),
        calculate_mean_rel_diff(
            sicelore_bulk_tpm_list[["MinION"]],
            sicelore_pseudobulk_tpm
        )
    )
)
...
```{r, message = FALSE, warning = FALSE}
results_df <- rbind(isosceles_mean_rel_diff, isoquant_mean_rel_diff,
                    flames_mean_rel_diff, sicelore_mean_rel_diff)
```

Helper function:
```{r}
plot_mean_rel_diff_top_n <- function(k_value) {
    plot_df <- list(
        data.frame(
            tool = "Isosceles",
    ...
    plot_df <- plot_df[!is.na(plot_df$mean_rel_diff),]
...
Bulk vs pseudobulk mean relative difference plot:
```{r, message = FALSE, warning = FALSE}
ggplot(results_df,
       mapping = aes(x = k_value,
                     y = mean_rel_diff,
                     col = tool,
                     linetype = platform)) +
    geom_line(size = 1) +
...
Bulk vs pseudobulk mean relative difference plot for top N transcripts (top 64 cells):
```{r, message = FALSE, warning = FALSE}
plot_mean_rel_diff_top_n(64)
```

Bulk vs pseudobulk mean relative difference plot for top N transcripts (top 800 cells):
```{r, message = FALSE, warning = FALSE}
plot_mean_rel_diff_top_n(800)
```
