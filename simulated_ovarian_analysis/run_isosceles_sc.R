...
sample_ids <- glue("{cell_line_ids}_Rep1")
bam_files <- as.character(glue("../input_data/bam_sim/{sample_ids}.bam"))
names(bam_files) <- sample_ids

result_dir <- "isosceles_results_sc"
...
# Prepare the pseudobulk SE objects
em.maxiter <- 250
em.conv <- 0.01
use_length_normalization <- FALSE
for (sample_id in sample_ids) {
    ...
    saveRDS(se_pseudobulk_tcc, file.path(
        result_dir, glue("{sample_id}_se_pseudobulk_tcc.rds")
    ))
    ...
    saveRDS(se_pseudobulk_transcript, file.path(
        result_dir, glue("{sample_id}_se_pseudobulk_transcript.rds")
    ))
}
